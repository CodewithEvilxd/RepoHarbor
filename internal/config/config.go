...
// Config stores RepoHarbor's persisted token and download directory.
type Config struct {
...
// Load reads the RepoHarbor config file from the user config directory.
...
// Save persists the RepoHarbor configuration to disk.
...
func ValidatePath(dir string) error {
	...
	testFile, err := os.CreateTemp(dir, ".repoharbor-write-check-*")
	if err != nil {
...
func CachePath() (string, error) {
	...
	return filepath.Join(base, "repoharbor", "cache"), nil
}
...
func path() (string, error) {
	...
	return filepath.Join(base, "repoharbor", "config.json"), nil
}

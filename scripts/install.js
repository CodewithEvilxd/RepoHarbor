...
const REPO = "codewithevilxd/repoharbor";
const PLATFORM_MAP = {
    darwin: "apple-darwin",
...
function binaryName() {
    return process.platform === "win32" ? "repoharbor.exe" : "repoharbor";
}
function downloadUrl() {
    const target = getTarget();
    const extension = process.platform === "win32" ? ".zip" : ".tar.gz";
    return `https://github.com/${REPO}/releases/download/v${VERSION}/repoharbor-${target}${extension}`;
}
function fetchStream(url) {
    return new Promise((resolve, reject) => {
        const request = https.get(url, { headers: { "User-Agent": "repoharbor-installer" } }, (response) => {
...
function buildFromSource(binaryPath) {
    (0, node_child_process_1.execFileSync)("go", ["build", "-o", binaryPath, "./cmd/repoharbor"], {
        stdio: "inherit",
    });
}
async function install() {
    ...
    console.log(`Downloading RepoHarbor from ${url}`);
    try {
        ...
        console.log("RepoHarbor installed successfully.");
        return;
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        console.warn(`Release download unavailable, building RepoHarbor from source: ${message}`);
        buildFromSource(binaryPath);
        fs.chmodSync(binaryPath, 0o755);
        console.log("RepoHarbor built from source successfully.");
    }
}
install().catch((error) => {
    const message = error instanceof Error ? error.message : String(error);
    console.error(`Failed to install RepoHarbor: ${message}`);
    process.exit(1);
});
